...
echo "$($PSQL "SELECT DISTINCT teams.name FROM games JOIN teams ON games.winner_id=teams.team_id OR games.opponent_id=teams.team_id WHERE year=2014 AND round='Eighth-Final' ORDER BY teams.name;")"
...
echo "$($PSQL "SELECT year, teams.name FROM games JOIN teams ON games.winner_id=teams.team_id WHERE round='Final' ORDER BY year;")"
